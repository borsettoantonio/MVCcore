...
namespace MyCourse.Models.Services.Infrastructure
{
        ...
        IUserConfirmation<ApplicationUser>
    {
        ...
        {
            DataSet dataSet = await db.QueryAsync($"SELECT * FROM AspNetUserClaims WHERE UserId={user.Id}");
        ...
        public async Task AddClaimsAsync(ApplicationUser user, IEnumerable<Claim> claims, CancellationToken token)
        {
            ...
            {
                int affectedRows = await db.CommandAsync($"INSERT INTO AspNetUserClaims (UserId, ClaimType, ClaimValue) VALUES ({user.Id}, {claim.Type}, {claim.Value})");
                if (affectedRows == 0)
                {
        ...
        public async Task RemoveClaimsAsync(ApplicationUser user, IEnumerable<Claim> claims, CancellationToken token)
        {
            foreach (Claim claim in claims)
            {
                int affectedRows = await db.CommandAsync($"DELETE FROM AspNetUserClaims WHERE UserId={user.Id} AND ClaimType={claim.Type} AND ClaimValue={claim.Value}");
                if (affectedRows == 0)
                {
        ...
        {
            DataSet dataSet = await db.QueryAsync($"SELECT AspNetUsers.* FROM AspNetUserClaims INNER JOIN AspNetUsers ON AspNetUserClaims.UserId = AspNetUsers.Id WHERE AspNetUserClaims.ClaimType={claim.Type} AND AspNetUserClaims.ClaimValue={claim.Value}");
        ...
        {
            DataSet dataSet = await db.QueryAsync($"SELECT * FROM AspNetUsers WHERE NormalizedEmail={normalizedEmail}");
            if (dataSet.Tables[0].Rows.Count == 0)
            {
                return null;
            }
        ...
        {
            int affectedRows = await db.CommandAsync($"REPLACE INTO AspNetUserTokens (UserId, LoginProvider, Name, Value) VALUES ({user.Id}, {loginProvider}, {name}, {value})");
            if (affectedRows == 0)
            {
        ...
        {
            int affectedRows = await db.CommandAsync($"DELETE FROM AspNetUserTokens WHERE UserId={user.Id} AND LoginProvider={loginProvider} AND Name={name}");
            if (affectedRows == 0)
            {
        ...
        {
            return db.QueryScalarAsync<string>($"SELECT Value FROM AspNetUserTokens WHERE UserId={user.Id} AND LoginProvider={loginProvider} AND Name={name}");
        }
        #endregion
        ...
        {
            int affectedRows = await db.CommandAsync($"INSERT INTO AspNetUserLogins (UserId, LoginProvider, ProviderKey, ProviderDisplayName) VALUES ({user.Id}, {login.LoginProvider}, {login.ProviderKey}, {login.ProviderDisplayName})");
            if (affectedRows == 0)
            {
        ...
        {
            int affectedRows = await db.CommandAsync($"DELETE FROM AspNetUserLogins WHERE UserId={user.Id} AND LoginProvider={loginProvider} AND ProviderKey={providerKey}");
            if (affectedRows == 0)
            {
        ...
        {
            DataSet dataSet = await db.QueryAsync($"SELECT * FROM AspNetUserLogins WHERE UserId={user.Id}");
        ...
        {
            DataSet dataSet = await db.QueryAsync($"SELECT AspNetUsers.* FROM AspNetUsers LEFT JOIN AspNetUserLogins ON AspNetUsers.Id=AspNetUserLogins.UserId WHERE LoginProvider={loginProvider} AND ProviderKey={providerKey}");
            if (dataSet.Tables[0].Rows.Count == 0)
            {
                return null;
            }
